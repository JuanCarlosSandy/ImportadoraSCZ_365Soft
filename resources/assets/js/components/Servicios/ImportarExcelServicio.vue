<template>
    <Dialog :visible.sync="dialogVisible" header="Importar productos" modal
        :containerStyle="{ width: '100%', maxWidth: '800px' }" @hide="cerrarModal">
        <template #header>
        ...
        <form @submit.prevent="submitForm">
            <div v-if="pageImportar == 0" class="text-center">
                <div v-if="!selectedFile" class="p-card p-component p-card-dashed p-mb-3" style="cursor: pointer">
                    <label class="custom-file" for="customFile">
                        ...
                        <p class="custom-file-label text-muted">
                            Seleccionar archivo Excel
                        </p>
                        <input type="file" class="custom-file-input" id="customFile" ref="fileInput"
                            accept=".xlsx" @change="handleFileChange" />
                    </label>
                </div>
                <div v-else class="p-mb-3">
                    <i class="pi pi-file-excel pi-2x p-text-success"></i>
                    <p class="p-mb-2">{{ selectedFile.name }}</p>
                    <Button icon="pi pi-trash" label="Eliminar archivo" class="p-button-danger p-button-sm"
                        @click="removeFile" />
                    <div class="p-mt-3" style="display:flex;justify-content:center;" v-if="
                        selectedFile.name
                            .split('.')
                            .pop()
                            .toLowerCase() == 'csv'
                    ">
                        ...
                        <Dropdown id="delimiterSelector" v-model="selectedDelimiter" :options="delimiters"
                            optionLabel="label" class="p-col-3" />
                    </div>
                    ...
                    <Button icon="pi pi-check" label="Confirmar" class="p-button-success p-button-sm p-mt-3"
                        @click="confirmCsv" />
                    </div>
                <Button icon="pi pi-download" label="Descargar plantilla Excel"
                    class="p-button-outlined p-button-sm p-button-primary" @click="downloadTemplate" />
            </div>
            <div v-if="pageImportar == 1">
                <Button icon="pi pi-check-square" label="Seleccionar Todos"
                    class="p-button-outlined p-button-sm p-button-success p-mb-3" @click="selectAllHeaders" />
                ...
                <div v-if="selectedHeadersFromFile.length > 0" style="overflow-x: auto;">
                    <DataTable :value="tableData" class="p-datatable-gridlines" responsiveLayout="scroll">
                        <Column v-for="(header, index) in headersOrigin" :key="index" :field="index.toString()"
                            :header="header">
                            <template #body="slotProps">
                                <span class="selected-header">{{
                                    slotProps.data[slotProps.field]
                                    }}</span>
                            </template>
                        </Column>
                    </DataTable>
                    <Button icon="pi pi-arrow-left" label="Volver" class="p-button-secondary p-button-sm p-mt-2"
                        @click="resetState" />
                    <Button icon="pi pi-check" label="Asignar Encabezados" class="p-button-success p-button-sm p-mt-2"
                        @click="assignHeaders" />
                </div>
            </div>
            <div v-if="pageImportar == 2">
                <div class="p-grid p-mt-4">
                    ...
                    <div class="p-col">
                        <Dropdown v-model="monedaSeleccionada" :options="arrayMonedas" optionLabel="nombre"
                            placeholder="Selecciona la moneda" />
                    </div>
                </div>
                <div class="p-mt-2" v-if="arrayMonedas.length != 0">
                    <div class="toolbar">
                        <Button icon="pi pi-arrow-left" label="Volver" class="p-button-secondary p-button-sm"
                            @click="resetState2" />
                        <Button icon="pi pi-download" label="Descargar"
                            class="p-button-outlined p-button-success p-button-sm" v-if="selectedFile"
                            @click="downloadCsv" />
                        <Button icon="pi pi-upload" label="Importar datos" class="p-button-success p-button-sm"
                            v-if="selectedFile && erroresPrevios.length == 0" type="submit" />
                    </div>
                </div>
                <p class="text-muted">
                    Este contenido se importará en la base de datos
                </p>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                <th v-for="(header, index) in headersOrigin" :key="index">
                                    {{ header }}
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(row, rowIndex) in previewCsv" :key="rowIndex">
                                <td v-for="(cell, cellIndex) in row" :key="cellIndex">
                                    {{
                                        [3,4,5].includes(cellIndex)
                                            ? cell + " " + monedaSeleccionada.simbolo
                                    : cell
                                    }}
                                </td>
                            </tr>
            ...
            <div v-if="pageImportar == 3">
                ...
                <div v-if="errorsImport.length > 0 && erroresNoExiste.length == 0">
                    <Message severity="error" icon="pi pi-exclamation-triangle" :closable="false">
                        ...
                        <p>
                            No se pudo realizar la importación, verifique los datos del
                            archivo
                        </p>
                    </Message>
                ...
                <div v-if="errorsImport.length != 0 && erroresNoExiste.length > 0">
                    <div v-if="registrosSuccess.length != 0">
                        ...
                        <div class="progress">
                            <div class="progress-bar" role="progressbar" :style="{
                                width:
                                    (registrosSuccess.length / erroresNoExiste.length) * 100 +
                                    '%',
                            }" :aria-valuenow="registrosSuccess.length" aria-valuemin="0" :aria-valuemax="erroresNoExiste.length">
                    ...
                    <DataTable :value="erroresNoExisteObject" v-if="registrosSuccess.length == 0"
                        responsiveLayout="scroll">
                        <h4>
                            <i class="fa fa-exclamation-circle"></i> Datos no encontrados en
                            la base de datos
                        </h4>
                    ...
                    <div v-if="erroresNoExiste.length > 0 && successImport.length != 0" class="mt-3">
                        ...
                        <Button label="Confirmar" icon="pi pi-check" class="p-button-success p-button-sm"
                ...
                " class="text-center">
                    ...
                    <h5 class="p-mt-3">
                        <i class="pi pi-check p-text-success"></i> Importación exitosa
                    </h5>
                </div>
            </div>
        </form>
...
export default {
    ...
    methods: {
        ...
        updateRegistrosSuccessObject() {
            this.registrosSuccessObject = this.registrosSuccess.map(
        ...
        convertArrayToObjectError() {
            ...
            this.erroresNoExisteObject = Object.entries(objeto).map(
                ([key, value]) => ({
                    info: `${key}: ${value}`,
                })
            );
        ...
        submitForm() {
            ...
            axios
                .post("/servicio/importar", formData)
                .then((response) => {
            ...
            for (let i = 0; i < row.length; i++) {
                ...
                if (char === "," && !withinQuotes) {
                    ...
                    if (columnCount === this.headersOrigin.length)
                        // Salir del bucle si ya hemos alcanzado las 5 columnas
                        break;
                } else if (char === '"') {
        ...
        downloadTemplate() {
            ...
            const nombreArchivo = "plantilla_servicios.xlsx";
        ...
        selectAllHeaders() {
            ...
            this.selectedHeadersFromFile = [
                ...this.csvHeaders.slice(0, this.headersOrigin.length),
            ];
            ...
            reader.onload = (event) => {
                ...
                if (this.selectedFile.name.endsWith(".csv")) {
                    ...
                    const arrayOfArrays = rows.map((row) =>
                        row.split(this.selectedDelimiter)
                    );
                    let newContent = this.getCsvSubset(
                        arrayOfArrays,
                        this.selectedHeadersFromFile
                    );
                    ...
                    const rows = content.split("\n");
                    const arrayOfArrays = rows.map((row, index) =>
                    ...
                    this.previewCsv = newContent;
                    this.previewCsvObject = this.convertArrayToObject(
                        this.selectedHeadersFromFile,
                        this.previewCsv
                    );
                    this.previewCsvObjectColumns = this.createColumns(
                        this.selectedHeadersFromFile
                    );
        ...
        getCsvSubset(csvContent, selectedHeaders) {
            ...
            const rows = Array.isArray(csvContent)
                ? csvContent
                : csvContent
                    .split("\n")
            ...
            const headerIndices = selectedHeaders.map((header) =>
                this.csvHeaders.indexOf(header)
            );
