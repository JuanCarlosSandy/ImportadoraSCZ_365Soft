<template>
  <main class="main">
    ...
    <Panel class="ingreso-panel">
      <template #header>
        <div class="panel-header">
          ...
          <h4 class="panel-title">COBROS</h4>
        </div>
      </template>
      ...
      <span class="badge bg-primary" id="cufdValor" style="display:none;" v-show="mostrarCUFD">No hay CUFD</span>-->
        ...
        <div>
          <DataTable :value="ventasAgrupadasPorCliente" paginator :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]"
            responsiveLayout="scroll" class="p-datatable-gridlines p-datatable-sm tabla-venta"
            paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown"
            currentPageReportTemplate="Mostrando {first} a {last} de {totalRecords}">
            <Column header="Opciones">
              <template #body="slotProps">
                <!-- Botón para ver ventas del cliente -->
                <Button icon="pi pi-eye" @click="verVentasCliente(slotProps.data)" class="p-button-sm p-mr-1 btn-mini"
                  style="backgroundColor: green; borderColor: green; color: white;" v-tooltip.top="'Ver'" />
                <!--<template v-if="slotProps.data.saldoRestanteSuma > 0">-->
                                  <template>

                  <Button label="Cobrar" icon="pi pi-wallet" class="p-button-sm p-button-warning p-mr-1 btn-mini"
                    @click="abrirModalCobroCliente(slotProps.data)" v-tooltip.top="'Cobrar'" />
                </template>
              </template>
            </Column>
            <Column field="razonSocial" header="Cliente"></Column>
            <Column field="documentoid" header="Documento"></Column>
            <Column header="Total Ventas">
              <template #body="slotProps">
                {{
                  (slotProps.data.totalSuma * parseFloat(monedaVenta[0])).toFixed(2)
                }}
                {{ monedaVenta[1] }}
              </template>
            </Column>
            <Column header="Saldo Restante">
              <template #body="slotProps">
                <span :class="{
                  'text-success fw-bold': slotProps.data.saldoRestanteSuma < 0,
                  'text-danger fw-bold': slotProps.data.saldoRestanteSuma > 0
                }">
                  {{ slotProps.data.saldoRestanteSuma.toFixed(2) }}
                </span>
              </template>
            </Column>
            <Column header="Estado">
              <template #body="slotProps">
                <span :class="getEstadoClassAgrupado(slotProps.data.saldoRestanteSuma)">
                  {{ getEstadoTextAgrupado(slotProps.data.saldoRestanteSuma) }}
                </span>
              </template>
            </Column>
            <Column header="Cant. Ventas">
              <template #body="slotProps">
                <span class="badge badge-info">
                  {{ slotProps.data.ventas.length }}
                </span>
              </template>
            </Column>
          </DataTable>
      ...
      <template v-else-if="listado == 2">
        <div class="detalle-venta-pro">
          ...
          <div v-if="idtipo_venta == 2 && cuotas && cuotas.length > 0" class="mt-4">
            ...
            <Button label="Modificar monto de cuota" icon="pi pi-pencil" class="p-button-sm p-button-warning mb-3"
            ...
            <DataTable :value="cuotas" class="p-datatable-sm p-datatable-gridlines">
                ...
                <template #body="slotProps">
                  ...
                  <div v-if="editarCuotas && slotProps.data.idtipo_pago !== 5">
                    <InputNumber v-model="slotProps.data.precio_cuota" mode="decimal" :min="0" :minFractionDigits="2"
                      ...
                      :class="{ 'p-warning': generaSaldoFavor(slotProps.data) }"
                      @blur="validarMontoCuota(slotProps.data)" />
                    <span class="ms-1">{{ monedaVenta[1] }}</span>
                    <!--<small v-if="generaSaldoFavor(slotProps.data)" class="text-danger d-block mt-1">
              ...
              <Column field="saldo_restante" header="Saldo Restante">
                <template #body="slotProps">
                  <span :class="{ 'text-success fw-bold': parseFloat(slotProps.data.saldo_restante) < 0 }">
              ...
              <Column field="estado" header="Estado">
                <template #body="slotProps">
                  ...
                  <!-- Saldo a Favor: cuando el cliente pagó de más -->
                  <span
                    v-else-if="parseFloat(slotProps.data.saldo_restante) < 0 || slotProps.data.estado === 'Saldo a Favor'"
              ...
              <Column header="Tipo de Pago">
                <template #body="slotProps">
                  ...
                  <div v-else>
                    ...
                    <span v-else-if="slotProps.data.idtipo_pago === 4" class="badge bg-info">
                      📤 Gastos
                    </span>
      ...
      <template v-else-if="listado == 3">
        <div class="detalle-venta-pro">
          ...
          <div class="detalle-header-pro">
            <div class="detalle-section-pro">
              <h3 class="detalle-titulo-pro">Historial de Cobros</h3>
              <p class="detalle-subtitulo-pro">Todas las cuotas y pagos del cliente</p>
            </div>
            <div class="detalle-meta-pro">
              <div>
                <span class="label-pro">Cliente</span>
                <p class="valor-pro">{{ clienteHistorial.razonSocial }}</p>
              </div>
              <div>
                <span class="label-pro">Documento</span>
                <p class="valor-pro">{{ clienteHistorial.documentoid }}</p>
              </div>
            </div>
          </div>

          <!-- RESUMEN -->
          <div v-if="resumenCuotas" class="mb-3 p-3 border rounded bg-light">
            <div class="d-flex justify-content-between flex-wrap gap-3">
              <div>
                <strong>Saldo Pendiente:</strong> 
                <span :class="{ 'text-danger fw-bold': parseFloat(resumenCuotas.saldo_pendiente) > 0 }">
                  {{ resumenCuotas.saldo_pendiente || 0 }} {{ monedaVenta[1] }}
                </span>
              </div>
            </div>
          </div>

          <!-- TABLA GENERAL DE CUOTAS -->
          <div class="detalle-tabla-pro">
            <DataTable
              :value="todasLasCuotasCliente"
            ...
            >
              ...
              <!-- N° COMPROBANTE -->
              <Column field="num_comprobante" header="N° Comprobante" style="width: 120px;">
                <template #body="slotProps">
                  <span class="badge bg-secondary">{{ slotProps.data.num_comprobante }}</span>
                </template>
              </Column>

              <!-- FECHA PAGO -->
              <Column field="fecha_pago" header="Fecha Pago" style="width: 150px;">
                <template #body="slotProps">
                  <!-- MODO EDICIÓN -->
                  <div v-if="slotProps.data.editando && slotProps.data.idtipo_pago !== 5">
                    <input
                      type="date"
                      class="form-control form-control-sm"
                      style="max-width: 140px"
                      :value="formatearFechaParaInput(slotProps.data.fecha_pago)"
                      @input="slotProps.data.fecha_pago = $event.target.value"
                    />
                  </div>
                  <!-- MODO LECTURA -->
                  <div v-else>
              ...
              <!-- MONTO CUOTA -->
              <Column field="precio_cuota" header="Monto Cuota" style="width: 150px;">
                <template #body="slotProps">
                  <!-- MODO EDICIÓN -->
                  <div v-if="slotProps.data.editando && slotProps.data.idtipo_pago !== 5">
                    <InputNumber 
              ...
              <!-- SALDO RESTANTE -->
              <Column field="saldo_restante" header="Saldo Restante" style="width: 130px;">
                <template #body="slotProps">
                  <span :class="{ 'text-success fw-bold': parseFloat(slotProps.data.saldo_restante) < 0 }">
                    {{ slotProps.data.saldo_restante }} {{ monedaVenta[1] }}
                  </span>
              ...
              <Column field="estado" header="Estado" style="width: 120px;">
                <template #body="slotProps">
                  ...
                  <span v-else-if="parseFloat(slotProps.data.saldo_restante) < 0 || slotProps.data.estado === 'Saldo a Favor'" class="badge bg-success">
                    💰 Saldo a Favor
                  </span>
              ...
              <!-- TIPO DE PAGO -->
              <Column header="Tipo de Pago" style="width: 200px;">
                <template #body="slotProps">
                  <!-- MODO EDICIÓN -->
                  <div v-if="slotProps.data.editando" class="d-flex flex-column gap-1">
                    <Dropdown
                      ...
                      class="p-inputtext-sm"
                    />
                    <span v-else class="badge bg-info p-2 text-center">🔒 Liquidación</span>
                    <Dropdown
                  ...
                  <!-- MODO LECTURA -->
                  <div v-else>
                    <span v-if="slotProps.data.idtipo_pago === 1" class="badge bg-success">💵 Efectivo</span>
                    <span v-else-if="slotProps.data.idtipo_pago === 5" class="badge bg-info">🔒 Liquidación</span>
                    <span v-else-if="slotProps.data.idtipo_pago === 4" class="badge bg-info">📤 Gastos</span>
                    <span v-else-if="slotProps.data.idtipo_pago === 7" class="badge bg-primary">🏦 {{ slotProps.data.nombre_cuenta || 'Cuenta bancaria' }}</span>
                    <span v-else class="badge bg-dark">No definido</span>
                  </div>
                </template>
              </Column>

              <!-- COLUMNA ACCIONES -->
              <Column header="Acciones" style="width: 130px; text-align: center;">
                <template #body="slotProps">
                  <div class="d-flex gap-1 justify-content-center">
                    <!-- BOTÓN EDITAR -->
                    <Button
                      v-if="!slotProps.data.editando && slotProps.data.idtipo_pago !== 5"
                      icon="pi pi-pencil"
                      class="p-button-sm p-button-warning btn-mini"
                      title="Editar cuota"
                      @click="activarEdicionCuota(slotProps.data)"
                    />
                    <!-- BOTÓN GUARDAR -->
                    <Button
                      v-if="slotProps.data.editando"
                      icon="pi pi-check"
                      class="p-button-sm p-button-success btn-mini"
                      title="Guardar cambios"
                      @click="guardarCuotaIndividual(slotProps.data)"
                    />
                    <!-- BOTÓN CANCELAR -->
                    <Button
                      v-if="slotProps.data.editando"
                      icon="pi pi-times"
                      class="p-button-sm p-button-secondary btn-mini"
                      title="Cancelar edición"
                      @click="cancelarEdicionCuota(slotProps.data)"
                    />
                    <!-- BOTÓN ELIMINAR -->
                    <Button
                      v-if="slotProps.data.idtipo_pago !== 5"
                      ...
                      @click="abrirModalEliminarCuota(slotProps.data)"
                    />
                    <span v-if="slotProps.data.idtipo_pago === 5" class="text-muted">-</span>
                  </div>
                </template>
              </Column>
          ...
          <!-- BOTÓN CERRAR -->
          <div class="detalle-footer-pro">
            <Button @click="cerrarHistorialCobros()" label="Volver" icon="pi pi-arrow-left" 
              class="p-button-secondary p-button-sm btn-mini" />
          </div>
        </div>
      </template>
    </Panel>
            ...
            <div class="p-grid p-formgrid align-items-start">
              ...
              width: 100%; max-height: 200px; overflow-y: auto; margin-top: 2px; border-radius: 4px; padding: 0;">
                    ...
                    <div class="p-mb-3">
                      ...
                      <div class="input-con-desplegable">
                        <InputText v-model="direccionCliente" class="input-full" :disabled="!direccionClienteEditable"
                          :class="{ 'p-invalid': errores.direccion }" @input="buscarDireccion"
                          @keydown.down="moverSeleccionDireccion('abajo')"
                          @keydown.up="moverSeleccionDireccion('arriba')" @keydown.enter="seleccionarDireccionEnter" />
                    ...
                    <div class="card mb-2" style="font-size: 0.8rem;">
                      <div class="card-body d-flex flex-column">
                        ...
                        <div class="form-group mb-0">
                          ...
                          <input type="text" id="cambioRecibir" class="form-control input-cambio bg-light"
                            :value="calcularTotal * parseFloat(monedaVenta[0]) - recibido" readonly />
                        </div>
                      </div>
                    </div>
      ...
      <Dialog :visible.sync="modalCobro" :modal="true" :closable="true" :blockScroll="true"
        ...
        <template v-if="ventaSeleccionada">
          ...
          <template slot="header">
            <div class="modal-header">
              <h5 class="modal-title">{{ esCobroCliente ? 'Cobro General del Cliente' : 'Cobro de Cuota' }}</h5>
          ...
          <div class="p-fluid">
            ...
            <div class="p-mb-3">
              ...
              <strong>Total Crédito:</strong> {{ parseFloat(ventaSeleccionada.total).toFixed(2) }} {{ monedaVenta[1] }}
              <br>
              <strong>Saldo Pendiente:</strong>
              <span :class="{ 'text-danger fw-bold': parseFloat(ventaSeleccionada.saldo_restante) > 0 }">
                {{ parseFloat(ventaSeleccionada.saldo_restante).toFixed(2) }} {{ monedaVenta[1] }}
              </span>
              <br>
              <template v-if="esCobroCliente">
                <small class="text-muted">
                  <i class="pi pi-info-circle mr-1"></i>
                  El pago se distribuirá automáticamente entre {{ ventaSeleccionada.ventas ?
                    ventaSeleccionada.ventas.length : 0 }} venta(s) pendiente(s)
                </small>
              </template>
            </div>
    ...
    <!-- 🔹 MODAL PARA VER VENTAS DEL CLIENTE -->
    <template>
      <Dialog :visible.sync="modalVentasCliente" :modal="true" :closable="true" :blockScroll="true"
        :containerStyle="{ width: '900px', maxWidth: '95vw' }">

        <template slot="header">
          <div class="modal-header">
            <h5 class="modal-title">
              Ventas de {{ clienteSeleccionado ? clienteSeleccionado.razonSocial : '' }}
            </h5>
            <button class="close-button" @click="modalVentasCliente = false">
              <i class="pi pi-times"></i>
            </button>
          </div>
        </template>

        <div class="p-fluid" v-if="clienteSeleccionado">
          <!-- Resumen del cliente -->
          <div class="card mb-3" style="background: #f8fafc;">
            <div class="card-body py-2">
              <div class="row">
                <div class="col-md-4">
                  <strong>Documento:</strong> {{ clienteSeleccionado.documentoid }}
                </div>
                <div class="col-md-4">
                  <strong>Total Ventas:</strong>
                  <span class="text-primary fw-bold">{{ clienteSeleccionado.totalSuma.toFixed(2) }} {{ monedaVenta[1]
                    }}</span>
                </div>
                <div class="col-md-4">
                  <strong>Saldo Restante:</strong>
                  <span
                    :class="{ 'text-success': clienteSeleccionado.saldoRestanteSuma < 0, 'text-danger': clienteSeleccionado.saldoRestanteSuma > 0 }">
                    {{ clienteSeleccionado.saldoRestanteSuma.toFixed(2) }} {{ monedaVenta[1] }}
                  </span>
                </div>
              </div>
            </div>
          ...
          <DataTable :value="ventasClienteSeleccionado" class="p-datatable-sm" responsiveLayout="scroll">
            <Column field="num_comprobante" header="N° Comprobante"></Column>
            <Column field="fecha_hora" header="Fecha"></Column>
            <Column header="Total">
              <template #body="slotProps">
                {{ parseFloat(slotProps.data.total).toFixed(2) }} {{ monedaVenta[1] }}
              </template>
            </Column>
            <Column header="Saldo">
              <template #body="slotProps">
                <span v-if="slotProps.data.idtipo_venta == 1">0</span>
                <span v-else
                  :class="{ 'text-success': parseFloat(slotProps.data.saldo_restante) < 0, 'text-danger': parseFloat(slotProps.data.saldo_restante) > 0 }">
                  {{ slotProps.data.saldo_restante || 0 }}
                </span>
              </template>
            </Column>
            <Column header="Tipo">
              <template #body="slotProps">
                <span class="badge" :class="slotProps.data.idtipo_venta == 1 ? 'badge-contado' : 'badge-credito'">
                  {{ slotProps.data.idtipo_venta == 1 ? 'Contado' : 'Crédito' }}
                </span>
              </template>
            </Column>
            <Column header="Acciones">
              <template #body="slotProps">
                <Button icon="pi pi-eye" class="p-button-sm p-button-info p-mr-1" @click="verVenta(slotProps.data.id)"
                  v-tooltip.top="'Ver'" />
                <Button v-if="slotProps.data.idtipo_venta == 2 && parseFloat(slotProps.data.saldo_restante) > 0"
                  icon="pi pi-minus-circle" class="p-button-sm p-button-danger p-mr-1" title="Registrar Gasto"
                  @click="abrirModalGasto(slotProps.data)" v-tooltip.top="'Gasto'" />
              </template>
            </Column>
          </DataTable>
        </div>
        ...
        <template #footer>
          <Button label="Cerrar" icon="pi pi-times" @click="modalVentasCliente = false" class="p-button-secondary" />
          <Button
            label="Ver todas las cuotas"
            icon="pi pi-list"
            class="p-button-info"
            @click="abrirDialogCuotasCliente(clienteSeleccionado.idcliente, clienteSeleccionado)"
          />
        </template>
      </Dialog>
    </template>
    ...
    <Dialog header="Registrar Gasto" :visible.sync="modalGastoVisible" modal :containerStyle="{ width: '400px' }">
      ...
      <div class="p-fluid">
        ...
        <div class="field mb-3">
          ...
          <InputNumber v-model="gasto.monto" mode="decimal" :min="0" :minFractionDigits="2" :maxFractionDigits="2"
        ...
        <div class="field mb-3">
          ...
          <InputText v-model="gasto.descripcion" rows="3" autoResize placeholder="Descripción del gasto" />
        </div>
      </div>

      <!-- BOTONES -->
      <template #footer>
        <Button label="Cancelar" icon="pi pi-times" class="p-button-text" @click="modalGastoVisible = false" />

        <Button label="Guardar Gasto" icon="pi pi-check" class="p-button-danger" @click="guardarGasto" />
      </template>
...
export default {
  ...
  data() {
    return {
      dialogCuotasCliente: false,
    cuotasPorVenta: [],
    resumenCuotas: null,
      modalGastoVisible: false,
      ventaSeleccionadaGasto: null,
      clienteHistorial: {
        razonSocial: '',
        documentoid: '',
        idcliente: null
      ...
      // 🔹 Variables para vista agrupada por cliente
      clienteSeleccionado: null,
      ventasClienteSeleccionado: [],
      modalVentasCliente: false,
      esCobroCliente: false, // 🔹 Para distinguir si es cobro de cliente agrupado
      clienteCobroData: null, // 🔹 Datos del cliente para cobro agrupado
  ...
  computed: {
    todasLasCuotasCliente() {
      const cuotas = [];
      this.cuotasPorVenta.forEach(venta => {
        if (venta.cuotas && venta.cuotas.length > 0) {
          venta.cuotas.forEach(cuota => {
            // Agregamos propiedades adicionales directamente al objeto original
            cuota.num_comprobante = venta.num_comprobante;
            cuota.fecha_venta = venta.fecha;
            cuota.total_venta = venta.total;
            cuota.idventa = venta.idventa;
            cuotas.push(cuota);
          });
        }
      });
      // Ordenar por fecha de pago (más antigua primero)
      return cuotas.sort((a, b) => {
        const fechaA = a.fecha_pago ? new Date(a.fecha_pago) : new Date(0);
        const fechaB = b.fecha_pago ? new Date(b.fecha_pago) : new Date(0);
        return fechaA - fechaB;
      });
    },
    // 🔹 Agrupar ventas por cliente
    ventasAgrupadasPorCliente() {
      const grupos = {};

      this.arrayVenta.forEach(venta => {
        const clienteKey = venta.documentoid || venta.razonSocial || 'SIN_CLIENTE';

        if (!grupos[clienteKey]) {
          grupos[clienteKey] = {
            id: venta.id, // ID de la primera venta (para referencia)
        ...
        grupos[clienteKey].totalSuma += parseFloat(venta.total) || 0;

        // 🔹 Solo sumar saldos positivos de ventas a CRÉDITO (deudas pendientes)
        // No incluir saldos negativos (saldos a favor) en este cálculo
        if (venta.idtipo_venta == 2) {
          const saldoVenta = parseFloat(venta.saldo_restante) || 0;
          // Solo sumar si es una deuda (saldo positivo)
          if (saldoVenta > 0) {
            grupos[clienteKey].saldoRestanteSuma += saldoVenta;
          }
        }
        ...
        if (venta.idtipo_venta == 2) {
          grupos[clienteKey].tieneCredito = true;
        }
      });
  ...
  methods: {
    ...
    // Si viene con timestamp (YYYY-MM-DD HH:mm:ss)
    return fecha.substring(0, 10);
  },

  // 🔹 Cerrar historial de cobros y volver al listado
  cerrarHistorialCobros() {
    this.listado = 1;
    this.cuotasPorVenta = [];
    this.resumenCuotas = null;
    this.clienteHistorial = { razonSocial: '', documentoid: '', idcliente: null };
  },

  async abrirDialogCuotasCliente(idCliente, clienteData = null) {
  try {
    this.isLoading = true;
    console.log("Cargando cuotas para el cliente ID:", idCliente);
    const res = await axios.get(`/ventas/cuotas-cliente/${idCliente}`);
    ...
    this.cuotasPorVenta = res.data.data;       
    this.resumenCuotas = res.data.resumen;
    
    // Guardar datos del cliente para el encabezado
    if (clienteData) {
      this.clienteHistorial = {
        razonSocial: clienteData.razonSocial || '',
        documentoid: clienteData.documentoid || '',
        idcliente: idCliente
      };
    } else if (this.clienteSeleccionado) {
      this.clienteHistorial = {
        razonSocial: this.clienteSeleccionado.razonSocial || '',
        documentoid: this.clienteSeleccionado.documentoid || '',
        idcliente: idCliente
      };
    }
...
// 🔹 Activar edición de una cuota individual
activarEdicionCuota(cuota) {
  // Guardar valores originales para poder cancelar
  this.$set(cuota, 'originalData', {
    fecha_pago: cuota.fecha_pago,
    precio_cuota: cuota.precio_cuota,
    idtipo_pago: cuota.idtipo_pago,
    bancoSeleccionado: cuota.bancoSeleccionado
  });
  this.$set(cuota, 'editando', true);
  // Forzar actualización
  this.$forceUpdate();
},

// 🔹 Cancelar edición de una cuota
cancelarEdicionCuota(cuota) {
  if (cuota.originalData) {
    this.$set(cuota, 'fecha_pago', cuota.originalData.fecha_pago);
    this.$set(cuota, 'precio_cuota', cuota.originalData.precio_cuota);
    this.$set(cuota, 'idtipo_pago', cuota.originalData.idtipo_pago);
    this.$set(cuota, 'bancoSeleccionado', cuota.originalData.bancoSeleccionado);
    delete cuota.originalData;
  }
  this.$set(cuota, 'editando', false);
  this.$forceUpdate();
},

// 🔹 Guardar cuota individual
async guardarCuotaIndividual(cuota) {
  try {
    this.isLoading = true;
    ...
    const payload = {
      ...
      precio_cuota: cuota.precio_cuota,
      idtipo_pago: cuota.idtipo_pago,
      idbanco: cuota.bancoSeleccionado ? cuota.bancoSeleccionado.id : null
    };

    const response = await axios.put(`/cuota/actualizar/${cuota.id}`, payload);
    ...
    if (response.data.success) {
      ...
      // Recargar las cuotas del cliente para reflejar el nuevo saldo
      if (this.listado === 3 && this.clienteHistorial.idcliente) {
        await this.abrirDialogCuotasCliente(this.clienteHistorial.idcliente, this.clienteHistorial);
      }
      
      // También recargar la lista principal de ventas
      ...
      this.$toast.add({
        severity: 'error',
        summary: 'Error',
        detail: response.data.message || 'No se pudo actualizar la cuota',
        life: 3000
      });
    }
  } catch (error) {
    console.error('Error al guardar cuota:', error);
    const mensajeError = (error.response && error.response.data && error.response.data.message) || 'Error al actualizar la cuota';
    this.$toast.add({
      severity: 'error',
      summary: 'Error',
      detail: mensajeError,
    ...
    async guardarGasto() {
      ...
      if (!this.gasto.descripcion || !this.gasto.descripcion.trim()) {
        this.$toast.add({
          ...
          detail: 'Debe ingresar una descripción del gasto.',
          life: 3000
        });
        return;
      }
      ...
      try {
        ...
        // 📤 Enviar al backend
        const response = await axios.post('/venta/registrargastos', {
          idventa: this.ventaSeleccionadaGasto.id,
        ...
        if (response.data && response.data.success) {
          ...
          if (this.clienteSeleccionado) {
            this.clienteSeleccionado.saldoRestanteSuma += Number(this.gasto.monto);
          }


          // 🔄 Recargar lista principal de ventas
          await this.listarVenta(1, this.buscar, this.criterio);

          // 🔥 FORZAR ACTUALIZACIÓN DEL DIÁLOGO DE CLIENTE
          this.$nextTick(() => {
            if (!this.clienteSeleccionado) return;

            const documentoCliente = this.clienteSeleccionado.documentoid;

            // 💥 Romper referencias para forzar re-render
            this.clienteSeleccionado = null;
            this.ventasClienteSeleccionado = [];

            this.$nextTick(() => {
              const clienteActualizado = this.ventasAgrupadasPorCliente.find(
                c => c.documentoid === documentoCliente
              );

              if (clienteActualizado) {
                this.clienteSeleccionado = clienteActualizado;
                this.ventasClienteSeleccionado = [...clienteActualizado.ventas];
              }
            });
          });
        } else {
          throw new Error(response.data.error || 'Error al registrar el gasto');
        }
        ...
        this.$toast.add({
          severity: 'error',
          summary: 'Error',
          detail: error.message || 'No se pudo registrar el gasto.',
          life: 4000
        });
    ...
    abrirModalGasto(venta) {
      this.cargarBancos();
      this.bancoSeleccionado = null;
      this.ventaSeleccionadaGasto = venta;
      this.gasto.monto = null;
      this.gasto.descripcion = '';
      this.modalGastoVisible = true;
    },
    puedeCobrarVenta(venta) {
      // 1️⃣ Debe ser venta a crédito
      if (Number(venta.idtipo_venta) !== 2) return false;

      // 2️⃣ Debe estar en estado 2 (pendiente / activa)
      if (Number(venta.estado) !== 2) return false;
    },
    ...
    async confirmarEliminarCuota() {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          if (this.listado === 3 && this.clienteHistorial.idcliente) {
            // Estamos en el historial de cobros del cliente
            await this.abrirDialogCuotasCliente(this.clienteHistorial.idcliente, this.clienteHistorial);
          } else if (this.idVentaSeleccionada) {
            // Vista de cuotas de una venta específica
          ...
          this.$toast.add({
            severity: 'error',
            summary: 'Error',
            detail: response.data.message || 'No se pudo eliminar la cuota',
            life: 3000
          });
        }
        ...
        const mensajeError = (error.response && error.response.data && error.response.data.message) || 'Ocurrió un error al eliminar la cuota';
        this.$toast.add({
          severity: 'error',
          summary: 'Error',
          detail: mensajeError,
    ...
    cuotaExcedida(cuota) {
      ...
      if (monto < 0) return true; // solo error real

      return false; // permitir excedentes
    },
    generaSaldoFavor() {
      const suma = this.cuotas.reduce((acc, c) => acc + Number(c.precio_cuota || 0), 0);
      return suma > this.total;
    },
    ...
    validarMontoCuota(cuota) {
      let monto = Number(cuota.precio_cuota || 0);
      ...
      if (monto < 0) {
        this.$toast.add({
          severity: 'warn',
          summary: 'Monto inválido',
          detail: 'El monto de la cuota no puede ser negativo.',
          life: 3000
        });
        cuota.precio_cuota = 0;
        return;
      }

      // 🔔 Advertir si genera saldo a favor (NO bloquear)
      const sumaCuotas = this.cuotas.reduce((acc, c) => {
        return acc + Number(c.precio_cuota || 0);
      }, 0);

      if (sumaCuotas > this.total) {
        const saldoFavor = (sumaCuotas - this.total).toFixed(2);
        this.$toast.add({
          severity: 'info',
          summary: 'Saldo a favor',
          detail: `Se actualizo el saldo a favor del Cliente.`,
          life: 3500
        });
      }
    ...
    async registrarPagoCuota() {
      ...
      // 🔹 Si es cobro de cliente agrupado, usar la función especial
      if (this.esCobroCliente) {
        await this.registrarPagoCliente();
        return;
      }
    ...
    // 🔹 Nueva función para registrar pago distribuido entre ventas del cliente
    async registrarPagoCliente() {
      let me = this;
console.log(me.ventaSeleccionada);
      let montoRestanteFavor = parseFloat(this.recibidoCuota);
      // Determinar tipo de pago
      let idtipo_pago = 1; // Por defecto efectivo
      if (this.tipoPagoCuota === 'descuento') {
        idtipo_pago = 5;
      } else if (this.tipoPagoCuota === 'banco') {
        idtipo_pago = 7;
      }
      // Obtener las ventas con saldo pendiente ordenadas por fecha (más antigua primero)
      const ventasPendientes = this.ventaSeleccionada.ventas
        .filter(v => parseFloat(v.saldo_restante) > 0)
        .sort((a, b) => new Date(a.fecha_hora) - new Date(b.fecha_hora));

     if (ventasPendientes.length === 0) {
    // Registrar directamente el saldo a favor del cliente
    try {
        const res = await axios.post('/venta/abonar-cuota', {
            idventa: null,           // no hay venta
            idcliente: me.ventaSeleccionada.documentoid,
            monto: montoRestanteFavor,
            idtipo_pago: idtipo_pago,
            idbanco: me.idbanco || null
        });
        ...
        swal('Registro exitoso!', `Monto ${montoRestanteFavor} registrado como saldo a favor.`, 'success');

        me.modalCobro = false;
        me.esCobroCliente = false;
        me.clienteCobroData = null;
        me.recibidoCuota = 0;
        me.tipoPagoCuota = 'efectivo';
        ...
        console.error(error);
        swal('Error', 'Ocurrió un problema al registrar el pago.', 'error');
        return;
    }
}

      let montoRestante = parseFloat(this.recibidoCuota);
      let pagosRealizados = [];

      // Distribuir el pago entre las ventas
      // Si el monto es mayor al saldo total, el excedente va a la última venta (genera saldo a favor)
      for (let i = 0; i < ventasPendientes.length; i++) {
        const venta = ventasPendientes[i];
        if (montoRestante <= 0) break;

        const saldoVenta = parseFloat(venta.saldo_restante);
        const esUltimaVenta = (i === ventasPendientes.length - 1);

        // Si es la última venta, asignar todo el monto restante (puede generar saldo a favor)
        // Si no es la última, solo pagar hasta el saldo de esa venta
        const montoAPagar = esUltimaVenta ? montoRestante : Math.min(montoRestante, saldoVenta);
      ...
      try {
        // Realizar los pagos secuencialmente
        for (let pago of pagosRealizados) {
          const res = await axios.post('/venta/abonar-cuota', pago);
        ...
        // Calcular si hubo saldo a favor
        const saldoTotalCliente = parseFloat(this.ventaSeleccionada.saldo_restante);
        const montoPagado = parseFloat(this.recibidoCuota);
        const saldoAFavor = montoPagado > saldoTotalCliente ? (montoPagado - saldoTotalCliente).toFixed(2) : 0;

        let mensaje = `Se distribuyó el pago de ${this.recibidoCuota} entre ${pagosRealizados.length} venta(s)`;
        if (saldoAFavor > 0) {
          mensaje += `. Saldo a favor generado: ${saldoAFavor}`;
        }
        ...
        this.modalCobro = false;
        this.esCobroCliente = false;
        this.clienteCobroData = null;
    ...
    abrirModalCobro(venta) {
      this.esCobroCliente = false; // 🔹 Es cobro de venta individual
      this.clienteCobroData = null;
    ...
    prepararDatosVenta(idtipo_pago, idtipo_venta) {
      ...
      const datosComunes = {
        ...
        descuento_total: this.calcularDescuentoTotal(),
        total,
    ...
    // 🔹 Funciones para vista agrupada por cliente
    getEstadoTextAgrupado(saldoRestante) {
      if (saldoRestante < 0) {
        return '💰 Saldo a Favor';
    ...
    getEstadoClassAgrupado(saldoRestante) {
      return {
        'estado-badge': true,
        'estado-verde': saldoRestante <= 0,  // Pagado o Saldo a Favor
        'estado-amarillo': saldoRestante > 0  // Tiene crédito pendiente
      };
    },

    verVentasCliente(clienteData) {
      // Mostrar todas las ventas del cliente en un modal o vista detallada
      this.clienteSeleccionado = clienteData;
      this.ventasClienteSeleccionado = clienteData.ventas;
      this.modalVentasCliente = true;
    },

    abrirModalCobroCliente(clienteData) {
      // Abre el modal para cobrar al cliente con saldo pendiente agrupado
      this.esCobroCliente = true;
      this.clienteCobroData = clienteData;

      // Crear un objeto ventaSeleccionada con los datos agrupados del cliente
      this.ventaSeleccionada = {
        id: null, // No es una venta individual
        razonSocial: clienteData.razonSocial,
        documentoid: clienteData.documentoid,
        total: clienteData.totalSuma,
        saldo_restante: clienteData.saldoRestanteSuma,
        ventas: clienteData.ventas.filter(v => v.idtipo_venta == 2 && parseFloat(v.saldo_restante) > 0)
      };
