<template>
  <main class="main">
    ...
    <Panel>
      <template #header>
        <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
          <div style="display: flex; align-items: center; gap: 0.5rem;">
            ...
            <h4 class="panel-title" style="margin: 0;">KARDEX DE INVENTARIO</h4>
          </div>

          <div style="display: flex; gap: 0.5rem;">
            <Button icon="pi pi-file-pdf" label="PDF General" class="p-button-danger p-button-sm btn-full"
              @click="descargarPdfGeneral()" />

            <Button icon="pi pi-filter" :label="mostrarLabel ? 'Filtros' : ''"
              class="p-button-secondary p-button-sm btn-full"
      ...
      <template>
        <div>
          <div class="toolbar-container">
            <div class="search-bar">
              <span class="p-input-icon-left">
                ...
                <InputText v-model="globalFilter" placeholder="Buscar por nombre, codigo, categoria, almacen, sucursal"
                  class="p-inputtext-sm input-full" />
              </span>
            </div>
             <div v-if="appliedFilters.length" class="filters-container">
          ...
          <DataTable :value="sortedResultados" responsiveLayout="scroll"
            class="p-datatable-gridlines p-datatable-sm tabla-pro" paginator :rows="10"
            :rowsPerPageOptions="[10, 20, 50]" :filters="filters"
            :globalFilterFields="['codigo', 'nombre_producto', 'categoria', 'sucursal', 'almacen']">
            <!-- Columna de ACCIONES -->
            <Column header="ACCIONES" bodyStyle="text-align: center" style="min-width: 80px; width: 80px;">
              <template #body="slotProps">
                <div style="display: flex; justify-content: center; gap: 0.5rem;">
                  <Button type="button" icon="pi pi-eye" class="p-button-info p-button-text btn-mini"
                    title="Ver Detalle en Pantalla" @click="verDetalleMovimiento(slotProps.data)" />
                  <Button type="button" icon="pi pi-file-pdf" class="p-button-danger p-button-text btn-mini"
                    title="Descargar PDF Detallado" @click="descargarPdfDetallado(slotProps.data)" />
                </div>
              </template>
            </Column>

            <!-- Columnas de datos -->
            <Column field="codigo" header="CODIGO ITEM" filter filterPlaceholder="Buscar código" />
            <Column field="sucursal" header="SUCURSAL" filter filterPlaceholder="Buscar sucursal" />
            <Column field="almacen" header="ALMACÉN" filter filterPlaceholder="Buscar almacén" />
            <Column field="nombre_producto" header="ITEM" filter filterPlaceholder="Buscar item" />
            <Column field="unidad_envase" header="UNID. X CAJA" />
            <Column field="categoria" header="CATEGORÍA" filter filterPlaceholder="Buscar categoría" />
            <Column field="total_ventas_texto" header="TOTAL VENTAS" />
            <Column field="total_ingresos_texto" header="TOTAL COMPRAS" />
            <Column field="total_traspasos_entrada" header="TOTAL TRASPASO ENTRADA"/>
            <Column field="total_traspasos_salida" header="TOTAL TRASPASO SALIDA" />
        ...
        <div class="modal-content">
          ...
          <form @submit.prevent="enviarFormulario">
            <div class="modal-body">
              <!-- Primera fila: Almacén, Artículo, Categoría -->
              <div class="form-group row">
                <div class="col-12 col-md-4 mb-3">
                  <label class="font-weight-bold">Almacén
                    <span class="text-danger">OBLIGATORIO</span></label>
                  <div class="input-group">
                    <input class="form-control" type="text" placeholder="Seleccione un almacén" disabled
                      v-model="almacenseleccionado.nombre_almacen" />
                    <div class="input-group-append">
                      <button class="btn btn-primary" type="button" @click="abrirModal2('Almacen')">
                        ...
                      </button>
                    </div>
                  </div>
                ...
                <div class="col-12 col-md-4 mb-3">
                  ...
                  <div class="input-group">
                    <input class="form-control" type="text" placeholder="Seleccione un artículo" disabled
                      v-model="articuloseleccionada.nombre" />
                    <div class="input-group-append">
                      <button class="btn btn-primary" type="button" @click="abrirModal2('Articulo')">
                        ...
                      </button>
                    </div>
                  </div>
                ...
                <div class="col-12 col-md-4 mb-3">
                  ...
                  <div class="input-group">
                    <input class="form-control" type="text" placeholder="Seleccione una categoría" disabled
                      v-model="lineaseleccionada.nombre" />
                    <div class="input-group-append">
                      <button class="btn btn-primary" type="button" @click="abrirModal2('Lineas')">
                        ...
                      </button>
                    </div>
                  </div>
              ...
              <div class="form-group row">
                <div class="col-12 col-md-6 mb-3">
                  <label class="font-weight-bold">Fecha Inicio
                    <span class="text-danger">OBLIGATORIO</span></label>
                  <input class="form-control" type="date" v-model="fechaInicio" />
                </div>

                <div class="col-12 col-md-6 mb-3">
                  <label class="font-weight-bold">Fecha Fin
                    <span class="text-danger">OBLIGATORIO</span></label>
              ...
              <div class="col-md-6">
                <div class="input-group">
                  ...
                  <input v-if="tituloModal2 == 'Marcas'" type="text" v-model="buscarA"
                    @keyup="listarMarca(1, buscarA, criterioA)" class="form-control" placeholder="Texto a buscar" />
                  <input v-if="tituloModal2 == 'Industrias'" type="text" v-model="buscarA"
                    @keyup="listarIndustria(1, buscarA, criterioA)" class="form-control" placeholder="Texto a buscar" />
                  <input v-if="tituloModal2 == 'Lineas'" type="text" v-model="buscarA"
                    @keyup="listarLinea(1, buscarA, criterioA)" class="form-control" placeholder="Texto a buscar" />
                  <input v-if="tituloModal2 == 'Proveedors'" type="text" v-model="buscarA"
                    @keyup="listarproveedor(1, buscarA, criterioA)" class="form-control" placeholder="Texto a buscar" />
                  <input v-if="tituloModal2 == 'Grupos'" type="text" v-model="buscarA"
                    @keyup="listargrupo(1, buscarA, criterioA)" class="form-control" placeholder="Texto a buscar" />
                  ...
                  <input v-if="tituloModal2 == 'Almacen'" type="text" v-model="buscarA"
                    @keyup="listarAlmacen(1, buscarA, criterioA)" class="form-control" placeholder="Texto a buscar" />
                  <input v-if="tituloModal2 == 'Articulo'" type="text" v-model="buscarA"
                    @keyup="listarArticulo(1, buscarA, criterioA)" class="form-control" placeholder="Texto a buscar" />
              ...
              <table class="table table-bordered table-striped table-sm" v-if="tituloModal2 !== 'Grupos'">
                ...
                <tbody>
                  <tr v-for="arrayelemento in arrayBuscador" :key="arrayelemento.id">
                    ...
                    <td v-if="tituloModal2 == 'Almacen'" v-text="arrayelemento.nombre_almacen"></td>
                    <td v-else v-text="arrayelemento.nombre"></td>
    ...
    <!--Fin del modal-->
    <div class="modal fade" tabindex="-1" :class="{ 'mostrar': modalDetalleMovimientos }" role="dialog"
      style="display: none;" :style="modalDetalleMovimientos ? 'display: block; overflow-y: auto;' : ''"
      aria-hidden="true">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <ul class="nav nav-tabs mb-3">
              <li class="nav-item">
                <a class="nav-link" :class="{ active: tabActual === 'ventas' }" href="#"
                  @click.prevent="tabActual = 'ventas'">
                  <i class="icon-basket"></i> Ventas
                </a>
              </li>
              <li class="nav-item">
                <a class="nav-link" :class="{ active: tabActual === 'ingresos' }" href="#"
                  @click.prevent="tabActual = 'ingresos'">
                  <i class="icon-bag"></i> Compras
                </a>
              </li>
              <li class="nav-item">
                <a class="nav-link" :class="{ active: tabActual === 'ajustes' }" href="#"
            ...
            <div v-if="tabActual === 'ventas'">
              <DataTable key="dt-ventas" :value="detalleMovimientos.ventas" :paginator="true" :rows="5" size="small"
              ...
              <div v-if="detalleMovimientos.ventas.length === 0" class="text-center p-3">No hay ventas en este periodo.
              </div>
            </div>

            <div v-if="tabActual === 'ingresos'">
              <DataTable key="dt-ingresos" :value="detalleMovimientos.ingresos" :paginator="true" :rows="5" size="small"
                stripedRows responsiveLayout="scroll">
                <Column field="fecha_hora" header="Fecha"></Column>
                <Column field="num_comprobante" header="N° Comprobante"></Column>
              ...
              <div v-if="detalleMovimientos.ingresos.length === 0" class="text-center p-3">No hay compras en este
                periodo.
              </div>
            </div>

            <div v-if="tabActual === 'ajustes'">
              <DataTable key="dt-ajustes" :value="detalleMovimientos.ajustes" :paginator="true" :rows="5" size="small"
                stripedRows responsiveLayout="scroll">
                <Column field="fecha_hora" header="Fecha"></Column>
              ...
              <div v-if="detalleMovimientos.ajustes.length === 0" class="text-center p-3">No hay ajustes en este
                periodo.
              </div>
            </div>
            <div v-if="tabActual === 'traspasos'">
            <DataTable :value="detalleMovimientos.traspasos" :paginator="true" :rows="5" size="small" stripedRows responsiveLayout="scroll">
              <Column field="fecha_hora" header="Fecha"></Column>
              <Column header="Movimiento">
                <template #body="slotProps">
                  <span :class="slotProps.data.tipo_movimiento === 'Entrada' ? 'badge badge-success' : 'badge badge-danger'" style="font-size: 0.9em;">
              ...
              <Column header="Cantidad">
                <template #body="slotProps">
                  <span v-if="slotProps.data.tipo_movimiento === 'Entrada'" style="color: green; font-weight: bold;">
...
export default {
  ...
  appliedFilters() {
      ...
      if (this.almacenseleccionado && this.almacenseleccionado.id) {
        filters.push({
          label: "Almacén",
          value: this.almacenseleccionado.nombre_almacen || this.almacenseleccionado.nombre,
        });
      }
      ...
      if (this.sucursalseleccionada && this.sucursalseleccionada.id) {
        filters.push({
          label: "Sucursal",
          value: this.sucursalseleccionada.nombre,
        });
      }

      if (this.proveedorseleccionada && this.proveedorseleccionada.id) {
        filters.push({
          label: "Proveedor",
          value: this.proveedorseleccionada.nombre,
        });
      }
      ...
      if (this.lineaseleccionada && this.lineaseleccionada.id) {
        filters.push({
          label: "Categoría",
          value: this.lineaseleccionada.nombre,
        });
      }
      ...
      if (this.articuloseleccionada && this.articuloseleccionada.id) {
        filters.push({
          label: "Producto",
          value: this.articuloseleccionada.nombre_producto || this.articuloseleccionada.nombre,
        });
      }
      ...
      if (this.fechaInicio && this.fechaFin) {
        filters.push({
          label: "Período",
          value: `${this.fechaInicio} → ${this.fechaFin}`,
        });
  ...
  methods: {
    ...
    seleccionar(selected) {
      if (this.tituloModal2 == "Marcas") {
        ...
        if (selected.condicion == 1) {
          this.almacenseleccionado = selected;
        } else if (selected.condicion == 0) {
          this.advertenciaInactiva("Almacen");
        }
    ...
    listaReporte() {
      ...
      if (!me.almacenseleccionado || !me.almacenseleccionado.id) {
        swal({
          title: "Almacén no seleccionado",
          text: "Debe seleccionar un almacén para generar el reporte.",
          type: "warning",
        ...
        me.isLoading = false;
        return; // No continúa si no hay almacén
      }
    ...
    descargarPdfGeneral() {
      ...
      let fechaFinFormateada = me.fechaFin || 'todasLasFechas';

      // Si las fechas están vacías, usar valor por defecto
      if (!me.fechaInicio) {
        fechaInicioFormateada = 'todasLasFechas';
      }
      if (!me.fechaFin) {
      ...
      axios({
        ...
        .then(response => {
          ...
          // Mostrar éxito
          me.toastSuccess('PDF descargado correctamente');
        })
        .catch(error => {
          console.error('Error descargando PDF:', error);
          me.toastError('Error al generar el PDF');
        })
        .finally(() => {
          // Ocultar loading
          me.isLoading = false;
    ...
    descargarPdfDetallado(data) {
      ...
      if (!data.id_articulo || !data.id_almacen) {
        swal("Atención", "Faltan datos del producto para generar el reporte", "warning");
        return;
      }
      ...
      axios({
        ...
        .then(response => {
          ...
          let nombreProducto = (data.nombre_producto || 'reporte').replace(/[^a-zA-Z0-9_áéíóúÁÉÍÓÚÑñ-]/g, '');
          link.download = `Kardex_${nombreProducto}_${me.fechaInicio}_${me.fechaFin}.pdf`;
        ...
        .catch(error => {
          console.error('Error descargando PDF:', error);
          me.toastError('Error al generar el PDF. Verifica que todos los campos estén completos.');
        })
        .finally(() => {
          // Ocultar loading
          me.isLoading = false;
