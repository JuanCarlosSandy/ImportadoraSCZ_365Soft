...
class VentasDetalladasExport implements FromCollection, WithHeadings, WithColumnWidths, WithStyles, WithEvents
{
    ...
    public function collection()
    {
        ...
        foreach ($ventas as $venta) {
            ...
            // 🔹 Calcular descuento total por producto
            $descuentoTotalDetalle = 0;

            foreach ($venta->detalles as $dtemp) {
                $descuentoUnitario = $dtemp->descuento ?? 0;
                $descuentoTotalDetalle += $descuentoUnitario * $dtemp->cantidad;
            }
            ...
            $infoEconomica = "Total: " . number_format($venta->total, 2) .
                " | Desc. Adic: " . number_format($descuentoAdicionalAplicado, 2) .
                " (" . ($venta->idtipo_venta == 1 ? 'Contado' : 'Crédito') . ")";
            ...
            foreach ($venta->detalles as $d) {
                ...
                $rows->push([
                    ...
                    number_format($descuentoTotalProducto, 2), // 🔹 NUEVA COLUMNA
                    number_format($totalLinea, 2),             // 🔹 Subtotal real
                    ''
                ]);
            }
            // 🔹 Calcular descuento total por producto (ya lo tienes implícito)
// Ahora calcular descuento adicional real

            $descuentoTotalDetalle = 0;

            foreach ($venta->detalles as $dtemp) {
                $descuentoTotalDetalle += ($dtemp->descuento ?? 0) * $dtemp->cantidad;
            }

            $descuentoAdicionalAplicado = max(0, ($venta->descuento_total ?? 0) - $descuentoTotalDetalle);
            if ($venta->estado != 0) {

                // 🔹 Total real de la venta (restando descuento adicional)
                $totalRealVenta = $sumaSubtotalesVenta - $descuentoAdicionalAplicado;

                $this->totalVentasRegistradas += $totalRealVenta;
            }
        }
    ...
    public function headings(): array
    {
        return [
            ...
            ['Cant.', 'Código', 'Producto', 'P. Unitario', 'Descuento', 'Subtotal', 'Datos de Venta / Estado']
        ];
    }
                ...
                for ($row = 4; $row <= $highestRow; $row++) {
                    ...
                    if (is_string($valA) && str_contains($valA, 'VENTA #')) {
                        ...
                        if ($estado == 'Anulado') {
                            $bgColor = 'F4CCCC';
                            $textColor = 'CC0000';
                        } elseif (str_contains($estado ?? '', 'Saldo')) {
                            $bgColor = 'FFF2CC';
                        }
